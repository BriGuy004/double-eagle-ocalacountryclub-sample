...
const Entertainment = () => {
  ...
  const allEntertainmentOffers = offers.map(offer => ({
    offerId: offer.club_id,
    clubId: offer.club_id,
    brand: offer.name,
    title: offer.name,
    description: offer.description || `Exclusive entertainment at ${offer.name}`,
    discount: "Member Discount",
    discountText: offer.discount_text,
    images: [offer.offer_card_url || offer.hero_image_url].filter(Boolean),
    tags: ["Entertainment", offer.city && offer.state ? `${offer.city}, ${offer.state}` : ''].filter(Boolean),
  ...
  const locationProducts = selectedLocation === "All Cities" 
    ? allEntertainmentOffers 
    : allEntertainmentOffers.filter(offer => offer.majorCity === selectedLocation);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <CategoryNav selectedCategory="Entertainment" />
