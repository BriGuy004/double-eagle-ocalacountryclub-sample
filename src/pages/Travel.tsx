...
const Travel = () => {
  ...
  const allTravelOffers = offers.map(offer => ({
    offerId: offer.club_id,
    clubId: offer.club_id,
    brand: offer.name,
    title: offer.name,
    description: offer.description || `Exclusive travel experience at ${offer.name}`,
    discount: "Member Discount",
    discountText: offer.discount_text,
    images: [offer.offer_card_url || offer.hero_image_url].filter(Boolean),
    tags: ["Travel", offer.city && offer.state ? `${offer.city}, ${offer.state}` : ''].filter(Boolean),
  ...
  const locationProducts = selectedLocation === "All Cities" 
    ? allTravelOffers 
    : allTravelOffers.filter(offer => offer.majorCity === selectedLocation);
